...
def parallel_timegan(rank, world_size, ori_data, parameters, checkpoint_file='checkpoint.pth'):
    ...
    # Combined generator and supervisor optimizer for backpropagation in Phase 2
    ...
    for itt in range(start_epoch['joint'], iterations):
        ...
        for kk in range(2):
            for X_mb, T_mb in dataloader:  # training gen
                ...
                # Clear memory
                del Z_mb, H_hat, H_hat_supervise, X_hat
                torch.cuda.empty_cache()
        ...
        for X_mb, T_mb in dataloader:
            ...
            noise_fake = torch.normal(mean=0, std=0.2, size=H_hat.size()).to(device)
            noisyY_fake = discriminator(H_hat_supervise + noise_fake)
            noisyY_fake_gen = discriminator(H_hat + noise_fake)
            noisyY_real = discriminator(H_real + noise_real)
            ...
            # Clear memory
            del Z_mb, H_hat, H_hat_supervise, H_real, noise_real, noise_fake, noisyY_fake, noisyY_fake_gen, noisyY_real
            torch.cuda.empty_cache()
